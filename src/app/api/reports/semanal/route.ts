...
export async function GET(request: Request) {
  const rateLimit = withRateLimit(request, RATE_LIMITS.general, "reports:semanal");
  ...
  try {
    ...
    doc.setFontSize(10);
    doc.text(`Semana: ${formatFullDate(weekStart)} - ${formatFullDate(weekEnd)}`, 105, 28, { align: "center" });
